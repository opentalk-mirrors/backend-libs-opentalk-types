...
/// The `SelectionStrategy` configured by the moderator, which determins, how the next participant
/// is choosen
...
impl SelectionStrategy {
    /// Determins if `playlist` is used to describe pool of valid selection targets
    pub fn uses_playlist(self) -> bool {
        matches!(self, SelectionStrategy::Playlist)
    }

    /// Determins if the `allow_list` is used to describe pool of valid selection targets
