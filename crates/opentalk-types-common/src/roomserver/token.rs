...
/// The token that is required to start a roomserver signaling session
#[derive(
...
#[cfg_attr(feature = "utoipa", derive(utoipa::ToSchema), schema(example = json!(Token::example_data())))]
pub struct Token(Uuid);

impl Token {
    /// Create a ZERO token id, e.g. for testing purposes
